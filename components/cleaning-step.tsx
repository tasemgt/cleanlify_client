...
import type { ColumnInfo, CleaningMapping, CleaningData } from "@/app/page"
...
export function CleaningStep({ columns, selectedDomain, rawData, onContinue, onBack }: CleaningStepProps) {
  const [cleaningData, setCleaningData] = useState<CleaningData>({})
  const [editingValues, setEditingValues] = useState<{ [key: string]: string }>({})

  useEffect(() => {
    // Generate mock cleaning suggestions for each column
    const initialCleaningData: CleaningData = {}

    columns.forEach((column) => {
      const values = rawData.map((row) => row[column.name]).filter((v) => v && v.trim())
      const uniqueValues = [...new Set(values)]

      // Generate mock suggestions based on domain and common patterns
      const suggestions = generateCleaningSuggestions(uniqueValues, selectedDomain)
      initialCleaningData[column.name] = suggestions
    })

    setCleaningData(initialCleaningData)
  }, [columns, rawData, selectedDomain])

  const generateCleaningSuggestions = (values: string[], domain: string): CleaningMapping[] => {
    // Mock suggestion logic - in real app, this would call an AI service
    const suggestions: CleaningMapping[] = []
    ...
    values.forEach((value) => {
      ...
      // General cleaning
      suggested = suggested.replace(/\s+/g, " ") // Multiple spaces
      suggested = suggested.toLowerCase().replace(/\b\w/g, (l) => l.toUpperCase()) // Title case
